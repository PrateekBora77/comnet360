...
import { Plus, Search, Server, UserCheck, Star, MessageSquarePlus } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="space-y-5">
        {/* Service name badge */}
        <div className="flex items-center gap-2.5 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800 dark:text-white">{service?.name}</p>
            <p className="text-xs text-slate-400 dark:text-slate-500">{service?.type} service</p>
          </div>
        </div>
...
export default function ServicesPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`bg-white dark:bg-slate-800 dark:border-slate-700 rounded-xl border border-slate-200 p-4 flex flex-wrap gap-3${isEnterpriseUser ? ' hidden' : ''}`}>
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Assigned To</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filtered.map((svc) => (
                <tr key={svc.serviceId} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                  {/* Name + owner */}
                    ...
                    {isEnterpriseUser ? (
                      ...
                      hasRole('ADMIN') && svc.status !== 'DECOMMISSIONED' ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-xs text-violet-600 hover:text-violet-800 font-medium border border-dashed border-violet-300 hover:border-violet-500 rounded-md px-2 py-0.5 transition-colors"
                        >
                          <UserCheck className="w-3 h-3" /> Assign User
                        </button>
                      ) : (
      ...
      >
        <div className="space-y-4">
          <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg px-3 py-2.5 text-sm text-slate-600 dark:text-slate-300">
            <span className="font-medium text-slate-800 dark:text-white">{assignTarget?.name}</span>
            <span className="text-slate-400 dark:text-slate-500 ml-2">({assignTarget?.type})</span>
          </div>
