    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Owner', value: userNameMap[service.ownerUserId] ?? 'Unknown User' },
          ...
          <Card key={item.label} className="text-center">
            ...
            <p className="text-sm font-semibold text-slate-800 mt-1 truncate" title={item.value}>{item.value}</p>
          </Card>
        ))}
        <Card className="text-center col-span-2 md:col-span-4">
          <p className="text-xs text-slate-400 font-medium mb-1">Assigned Enterprise User</p>
      ...
      <div className="bg-white rounded-xl border border-sky-100 shadow-sm">
        ...
        {!configs || configs.length === 0 ? (
          ...
          <table className="w-full text-sm min-w-[400px]">
            <thead className="bg-slate-50 border-b border-sky-50">
              <tr>
            ...
            <tbody className="divide-y divide-sky-50">
              {configs.map((cfg) => (
                <tr key={cfg.configId} className="hover:bg-sky-50">
                  <td className="px-5 py-3 font-mono text-xs text-blue-700 font-medium">{cfg.parameter}</td>
                  <td className="px-5 py-3 text-slate-700 text-xs">{cfg.value}</td>
                  <td className="px-5 py-3 text-slate-400 text-xs">{cfg.effectiveDate}</td>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button loading={addConfigMut.isPending} disabled={!config.parameter.trim() || !config.value.trim()} onClick={() => addConfigMut.mutate()}>
              Save
            </Button>
          </>
        }
